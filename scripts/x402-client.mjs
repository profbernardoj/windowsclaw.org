...
function getPrivateKey() {
  ...
  try {
    const token = execSync(
      'security find-generic-password -a "everclaw-agent" -s "op-service-account-token" -w',
      { encoding: "utf-8", timeout: 5000 }
    ).trim();

    const key = execSync(
      `OP_SERVICE_ACCOUNT_TOKEN=${token} op item get "Base Session Key" --vault "Agent Vault" --fields "Private Key" --reveal`,
