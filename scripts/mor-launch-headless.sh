...
OP_TOKEN=$(security find-generic-password -a "everclaw-agent" -s "op-service-account-token" -w 2>/dev/null || true)
if [[ -n "$OP_TOKEN" ]]; then
  export OP_SERVICE_ACCOUNT_TOKEN="$OP_TOKEN"
  WALLET_KEY=$(op item get "Base Session Key" --vault "Agent Vault" --fields "Private Key" --reveal 2>/dev/null || true)
fi
