...
**Version 1.0** — Prevents premature execution, eliminates looping, forces structured problem-solving, ensures full completion. Applies to EVERY non-trivial task. No exceptions.

### CORE RULE
Before solving anything: **PLAN BEFORE EXECUTION.** No output generation until the task is decomposed.
...
List all assumptions explicitly. For each assumption:
- Is it confirmed?
- Is it inferred?
- Is it risky?
...
Solve ONE subtask at a time. For each subtask:
- Execute
- Validate
...
1. **Fix errors immediately.** Don't ask. Don't wait. If you see an error, fix it.
2. **Spawn subagents for execution.** Use `sessions_spawn` for coding tasks when possible.
3. **Never force push, delete branches, or rewrite git history.** Git history is sacred.
