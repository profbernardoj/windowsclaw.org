...
**Fix:**
1. Check your balance: `bash scripts/everclaw/scripts/balance.sh`
2. Close old/unused sessions to reclaim staked MOR
...
4. If corrupted, delete it and restart: `rm ~/morpheus/.cookie && bash scripts/start.sh`
...
## Proxy Not Showing All Models

**Cause:** The model refresh was parsing router response incorrectly (fixed in v2026.3.19).

**Fix:**
1. Update to EverClaw v2026.3.19 or later
2. Restart the proxy: `launchctl kickstart gui/$(id -u)/com.morpheus.proxy`
3. Check health: `curl http://127.0.0.1:8083/health | jq '.availableModels | length'`

Should show 40+ models including GLM-5.

---

## Fallback Mode Stuck

**Cause:** Proxy is routing through Gateway because P2P session failed.

**Fix:**
1. Check session status: `curl http://127.0.0.1:8083/health | jq '.activeSessions'`
2. Open a new session: `bash scripts/session.sh open glm-5 86400`
3. Check MOR balance: `bash scripts/balance.sh`

---

## All Providers in Cooldown

**Cause:** OpenClaw puts providers in cooldown after repeated failures.
...
4. ✅ Is `models-config.json` populated? `cat ~/morpheus/models-config.json | jq '.models | length'`
5. ✅ Do you have MOR? `bash scripts/balance.sh`
6. ✅ Do you have ETH for gas? (check balance output)
7. ✅ Is there an active session for your model? `bash scripts/session.sh list`
...
10. ✅ Run diagnostic: `bash scripts/diagnose.sh --verbose`
